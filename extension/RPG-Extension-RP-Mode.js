  ...
  return Promise.all([
    ...
    return lbStep.then(function (lbId) {
      ...
      /* Vector 9 (Phase 1): install bundle-shipped engine-native regex
         ...
         prefixed with `MRRP: <rulesetId>: <authorName>`. On re-install we
      ...
      var rulesetPrefix = MRRP_REGEX_NAME_PFX + rulesetId + ": ";
      ...
      /* Vector 3 (Phase 1): install bundle-shipped engine-native custom
         tools. Static-type tools only as of v0.2.x — script-type tools
         ...
         and enforces uniqueness via 409 — we use a `mrrp_<rulesetIdSnake>_`
      ...
      var rulesetToolPrefix = MRRP_TOOL_NAME_PFX + rulesetIdSnake + "_";
      ...
      return apiFetch("/custom-tools", {}).then(function (existing) {
        ...
        tools.forEach(function (t, i) {
          addChain = addChain.then(function () {
            ...
            var body = {
              name: fullName,
              description: typeof t.description === "string" ? t.description.slice(0, 500) : "MRRP-managed tool for " + rulesetId,
...
function uninstallBundleArtifacts(rulesetId, authorId, progressCb) {
  ...
  return Promise.all([
    ...
    var rulesetRegexPrefix = MRRP_REGEX_NAME_PFX + rulesetId + ": ";
...
function loadCharacters(chatId) {
  ...
  /* No mrrp-chars-<chatId> in localStorage → first-time character bootstrap.
  ...
  lsSet("mrrp-chars-" + chatId, JSON.stringify(fresh));
  lsSet("mrrp-active-char-" + chatId, fresh[0].id);
...
function mrrpP3RenderSheet() {
  ...
  sections.forEach(function (sec) {
    ...
    else if (sec === "meritsFlaws") mrrpP3RenderMeritsFlawsSection(state.mountEl);
...
function mrrpRenderResourceStateBanner(parent, resource) {
  var stateRef = (typeof resource.stateRef === "string") ? resource.stateRef : null;
  if (!stateRef) {
    mrrpRenderResourcePlaceholder(parent, resource, "(missing stateRef)");
    return;
  }
  var stateDef = null;
  ...
  if (!stateDef || !Array.isArray(stateDef.values) || !stateDef.values.length) {
    mrrpRenderResourcePlaceholder(parent, resource, "(state '" + stateRef + "' not found)");
    return;
  }
  ...
  /* Slug-form for class targeting so per-tier visual styling can attach
     (e.g. `.mrrp-state-banner__pill--bonfire`). Lowercase, non-alphanumeric
     → dash, trimmed of leading/trailing dashes. */
  function slug(s) {
    return String(s || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
  }
  var wrap = marinara.addElement(parent, "div", { "class": "mrrp-state-banner" });
  if (!wrap) return;
  var pill = marinara.addElement(wrap, "button", {
    "class": "mrrp-state-banner__pill mrrp-state-banner__pill--" + slug(current),
    "type": "button",
    title: "Click to cycle " + stateRef,
    textContent: current
  });
  if (!pill) return;
  var sel = marinara.addElement(wrap, "select", { "class": "mrrp-state-banner__select" });
  if (sel) {
    ...
    marinara.on(sel, "change", function () {
      ...
      pill.className = "mrrp-state-banner__pill mrrp-state-banner__pill--" + slug(sel.value);
    });
  }
  marinara.on(pill, "click", function (e) {
    ...
    pill.className = "mrrp-state-banner__pill mrrp-state-banner__pill--" + slug(nextLabel);
...
function mrrpP3RenderResourcesSection(parent) {
  ...
  resources.forEach(function (resource, idx) {
    ...
    var formulaMax = mrrpResolveResourceMax(resource, ctx);
    ...
    if (resource.type === "bar") {
      ...
      mrrpRenderResourceStateBanner(card, resource);
    } else {
...
function mrrpP3RenderDerivedSection(parent) {
  ...
  mrrpP3RenderSection(parent, {
    ...
    marinara.addElement(body, "div", {
      "class": "mrrp-derived-pools__subtitle",
      textContent: "Auto-calculated from your stats + equipped gear. GM can read these directly."
    });
    bars.forEach(function (d) { mrrpP3RenderDerivedBar(body, d); });
    tracks.forEach(function (d) { mrrpP3RenderDerivedTrack(body, d); });
    if (cards.length) {
      var grid = marinara.addElement(body, "div", { "class": "mrrp-derived-pools" });
      if (grid) {
        cards.forEach(function (d) { mrrpP3RenderDerivedPoolCard(grid, d); });
      } else {
        cards.forEach(function (d) { mrrpP3RenderDerivedPoolCard(body, d); });
      }
    }
  });
}
...
/* Phase 6 — Single value-card renderer. Replaces the old row-based
   ...
   .derived[d.name]. */
function mrrpP3RenderDerivedPoolCard(parent, d) {
  if (!parent || !d) return;
  var card = marinara.addElement(parent, "div", { "class": "mrrp-derived-pool-card" });
  if (!card) return;
  var head = marinara.addElement(card, "div", { "class": "mrrp-derived-pool-card__head" });
  if (!head) return;
  marinara.addElement(head, "div", {
    "class": "mrrp-derived-pool-card__name",
    textContent: d.name
  });
  var valueWrap = marinara.addElement(head, "div", { "class": "mrrp-derived-pool-card__value-wrap" });
  if (!valueWrap) return;
  ...
  if (isAutoCalc) {
    var valSpan = marinara.addElement(valueWrap, "span", {
      "class": "mrrp-derived-pool-card__value mrrp-derived-pool-card__value--autocalc",
      title: hasValueFormula
        ? ("Auto-calculated from formula: " + d.valueFormula)
        : ("Auto-calculated from formula: " + d.tooltipFormula)
    });
    bonusSpan = marinara.addElement(valueWrap, "span", { "class": "mrrp-derived-pool-card__bonus" });
    if (bonusSpan) {
    ...
    if (input) {
      input.classList.add("mrrp-derived-pool-card__value-input");
    }
    if (declaredMax != null) {
      marinara.addElement(valueWrap, "span", {
        "class": "mrrp-derived-pool-card__max",
  ...
  if (formulaText) {
    marinara.addElement(card, "div", {
      "class": "mrrp-derived-pool-card__formula",
  ...
  if (typeof d.rollFormula === "string" && d.rollFormula
    ...
    var rollBtn = marinara.addElement(card, "button", {
      "class": "mrrp-derived-pool-card__roll",
      textContent: "roll"
    });
    if (rollBtn) {
...
function mrrpP3RenderSkillsSection(parent) {
  ...
  mrrpP3RenderSection(parent, {
    ...
    if (groups && groups.length) {
      ...
      /* "Other" bucket — skills declared in skills[] but not listed in
         any group. Rare but possible if a ruleset author forgets to
         bucket a new skill. Keeps the skill visible instead of dropping it. */
...
function mrrpP3RenderMeritsFlawsSection(parent) {
  if (!parent || !state.ruleset) return;
  if (!Array.isArray(state.sheet.meritsFlaws)) state.sheet.meritsFlaws = [];
  ...
  mrrpP3RenderSection(parent, {
    ...
    ["merit", "flaw"].forEach(function (kind) {
      ...
      entries.forEach(function (entry) {
        ...
        var typeSel = marinara.addElement(row, "select", { "class": "mrrp-custom-skill-attr" });
        if (typeSel) {
...
function quickRollForDerived(derived) {
  if (!state.ruleset) return;
  var mode = state.ruleset.resolution.mode;
  if (!derivedRollSupported(mode)) return;
...
function renderValue(parent, derived) {
  ...
  if (hasValueFormula || tooltipOnlyAutocalc) {
    ...
    /* Roll button on autocalc'd derived stats that declare a rollFormula
       (Initiative = {Dexterity_mod} for D&D; {Wits}+{Awareness} for Exalted). */
...
function castAbilityPool(ability, catId) {
  ...
  /* Phase 6 — auto-deduct from sheet. Only fires for known mote /
     ...
     borrow this cast path (e.g. future indie ruleset adding charm-style
     abilities to single-roll mode) don't get surprise sheet edits. */
  ...
  if (cost && commitModel === "mote") {
    ...
    /* V20 + other rulesets — blood pool spend pattern. costText "1B" or
       "2 blood" subtracts from a "Blood Pool" derived bar. */
  ...
  /* Build the cast chat tag with everything the GM agent needs to
     resolve the activation: who cast (name), from where (discipline),
     at what rating (player's category score), and at what cost. */
  ...
  /* Post directly to the chat input. insertIntoChatInput finds the
     visible chat textarea, appends the tag, and fires input/change
     events so Marinara's chat composer picks up the new text. */
  var injected = false;
  if (typeof insertIntoChatInput === "function") {
    injected = !!insertIntoChatInput(tag);
  }
  if (!injected) {
    /* Fallbacks: clipboard so the player can paste it manually, then
       finalizeRoll so the tag at least shows up in the dice widget's
       result area (the legacy preview-then-send path). */
  ...
  /* Open the dice widget so the player can immediately roll their
     activation pool. For Exalted, the player picks the Attribute + Ability
     combination; pre-seeding rating gives a non-zero starting point. */
  ...
  /* Re-render the sheet so the deducted motes / WP are immediately
     visible in the Resources cluster. saveSheet already wrote; this
     just refreshes the pixels. */
  if (typeof renderSheet === "function") renderSheet();
}
...
function rollDicePoolSum() {
  ...
  var text = "[mrrp-roll: mode=dice-pool-sum pool=" + pool + " dieSize=" + dieSize +
             pipsText + wildText + " total=" + total + " vs " + diff +
             " => " + (pass ? "success" : "fail") + "]";

  var kind = critFail ? "botch" : (pass ? "success" : "fail");
  var renderFaces = faces.map(function (f) {
    return { face: f, cls: "mrrp-dice__face" };
  });
  if (wdOn) {
    renderFaces.push({
      face: wildFace,
      cls: "mrrp-dice__face mrrp-dice__face--wild" + (critFail ? " mrrp-dice__face--one" : "")
    });
    wildExplodes.forEach(function (xf) {
      renderFaces.push({ face: xf, cls: "mrrp-dice__face mrrp-dice__face--double" });
    });
  }
...
function showDice(open) {
  ...
  if (!state.diceEl) {
    try { console.error("[mrrp] showDice: buildDice failed; dice widget unavailable"); } catch (e) {}
    return;
  }
  if (open) {
    ...
    visibleDerivedForSnap.forEach(function (d) {
      ...
      if (typeof d.valueFormula === "string" && d.valueFormula) {
        ...
        base = (typeof vf === "number" && isFinite(vf)) ? Math.floor(vf) : null;
                 ...
                 && typeof mrrpComputeTooltipBreakdown === "function") {
        var brk = mrrpComputeTooltipBreakdown(d, ctx);
